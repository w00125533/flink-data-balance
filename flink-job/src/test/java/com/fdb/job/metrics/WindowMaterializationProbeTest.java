...
class WindowMaterializationProbeTest {
    @Test
    void emits_window_materialization_sample_with_window_end_in_window_kind() {
        ...
        StageMetricSample sample = WindowMaterializationProbe.sample(
            ...
            60_000L, 1_000L, metricConfig, 2, 123_456L);
    ...
    void emits_completed_window_only_after_a_newer_window_is_seen() {
        ...
        WindowMaterializationProbe<Object> probe = new WindowMaterializationProbe<>(
            "window-chr-1m", "CHR 1m Materialization", "chr-1m", "MIN_1",
            ignored -> 0L, metricConfig);

        assertThat(probe.recordWindowMaterialization(60_000L, 1_000L)).isEmpty();
        assertThat(probe.recordWindowMaterialization(60_000L, 1_001L)).isEmpty();

        assertThat(probe.recordWindowMaterialization(120_000L, 1_002L))
            .singleElement()
            .satisfies(sample -> {
                assertThat(sample.windowKind()).isEqualTo("MIN_1@60000");
        ...
        assertThat(probe.recordWindowMaterialization(120_000L, 1_003L)).isEmpty();
